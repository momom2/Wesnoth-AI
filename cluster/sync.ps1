# Sync code (and optionally the self-play checkpoint) to the ENSTA
# Mesogip cluster.
#
...
#   - With -IncludeCheckpoint: training/checkpoints/sim_selfplay.pt
#     (overwrites whatever's on the cluster).
#
# Does NOT touch on the cluster:
#   - replays_dataset/, replays_raw/  (corpus -- built once, ship via bundle)
#   - training/checkpoints/            (UNLESS -IncludeCheckpoint is set;
#                                       then sim_selfplay.pt is pushed and
#                                       overwrites the cluster's copy)
...
#   -IncludeCheckpoint   Also push `training/checkpoints/sim_selfplay.pt`
#                 in the tarball. The cluster's copy is OVERWRITTEN on
#                 extract. Used by the GUI's `Sync + Continue (self-
#                 play)` button so a local-overnight training session's
#                 progress lands on the cluster before the next chain
#                 link starts. Overwrite semantics by design (the
#                 alternative, mtime-comparison, is fragile across
#                 machines) -- if the local checkpoint is staler than
#                 the cluster's, the operator has chosen to discard
#                 the cluster's progress. Refuses to push if the
#                 local checkpoint doesn't exist.
...
# Vendored Wesnoth add-ons. We host third-party scenario packs that
# the trainer references (currently just Mini_Maps_Collection for
# the engagement-curriculum phase). These live under
# wesnoth_src/data/add-ons/ -- NOT in the standard multiplayer/
# tree the build_bundle ships. Sync them as discovered files so the
# cluster can resolve `{~add-ons/<pkg>/...}` map_data references.
$addonFiles = @()
if (Test-Path 'wesnoth_src/data/add-ons') {
    $addonFiles = @(
        Get-ChildItem -Path 'wesnoth_src/data/add-ons' -Recurse -File |
            ForEach-Object {
...
# Optional checkpoint push. We use the same tarball mechanism as code
# files so the byte-stream / checksum / atomic-extract guarantees
# apply equally. The cluster's existing sim_selfplay.pt is overwritten
# on extract -- by design (operator chose to push, accepting that
# whatever the cluster had is discarded). Warn loudly in the log so
# this is never silent.
$checkpointPath = 'training/checkpoints/sim_selfplay.pt'
if ($IncludeCheckpoint) {
    if (Test-Path $checkpointPath) {
        $sz = (Get-Item $checkpointPath).Length
        $mtime = (Get-Item $checkpointPath).LastWriteTime.ToString('yyyy-MM-dd HH:mm:ss')
        Write-Host ("[sync] -IncludeCheckpoint: WILL OVERWRITE cluster's sim_selfplay.pt " +
                    "with local copy ({0:N1} MB, mtime {1})" -f ($sz / 1MB), $mtime)
        $extras += $checkpointPath
    } else {
        # Refuse to silently skip -- the operator asked for a push;
        # the absence of the file is a configuration error worth
        # surfacing rather than swallowing.
        throw "[sync] -IncludeCheckpoint requested but $checkpointPath not found locally. Run local self-play training first, or omit the flag."
    }
}

$paths = $rootPy + $toolsPy + $clusterFiles + $configFiles + $extras + $addonFiles
...
if ($Continue) {
    Write-Host "[sync] new $Mode job submitted -- watch with: ssh $RemoteHost 'bash cluster/run.sh tail $Mode'"
} else {
