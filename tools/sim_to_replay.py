...
def _per_scenario_template_path(scenario_id: str) -> Path:
    ...
    [scenario] block extracted from a real Wesnoth save (post-macro
    expansion). Generated by `tools/extract_scenario_templates.py`,
    committed to the repo, consumed by the runtime emitter."""
...
def build_save_wml(
    ...
    *,
    ...
    No `replays_raw/` at runtime. The per-scenario templates are
    repo-tracked outputs of one-shot
    `tools/extract_scenario_templates.py` runs, so the chain is:
    wesnoth_src + replays_raw -> extract -> templates -> runtime.
    """
    ...
    if not template_path.exists():
        raise RuntimeError(
            ...
            f"Run `python tools/extract_scenario_templates.py` "
            f"to regenerate from wesnoth_src + replays_raw/."
        )
    ...
    # Pre-placed villages also from the .cfg.
