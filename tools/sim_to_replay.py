...
- ATTACK commands carry FULL `[checkup]` blocks (per-strike
  ...
  playback isn't possible on the stock binary, see BACKLOG
  "Wesnoth-playback verification harness"). Other command kinds
  still emit empty `[checkup]` acks (structurally required, verify
  nothing).
- We DO emit `[random_seed]` dependent commands after each recruit /
  ...
  matches what the sim rolled internally (wesnoth_sim.request_seed),
  so playback reproduces the sim's trait / damage rolls bit-exact.
...
def _wml_bool(v: bool) -> str:
    """Engine WML-writer bool serialization (yes/no). The checkup
    comparison round-trips through this exact form in real saves."""
...
def _wml_for_command(rc: RecordedCommand,
    ...
    if rc.kind == "attack":
        ...
        # VERIFIABLE checkup: emit the per-strike [result] children
        # the engine itself would have recorded (two per strike:
        # {chance,hits,damage} then {dies} -- attack.cpp 1.18.4).
        # Wesnoth's synced_checkup COMPARES stored [result]s against
        # its own calculations at playback and raises the SYNC error
        # on mismatch; an EMPTY [checkup] would instead silently
        # append (recorder mode) and verify nothing
        # (synced_checkup.cpp::local_checkup). Bools as yes/no and
        # bare decimal ints, matching the engine's own WML writer.
        checkup = ""
        for entry in (rc.extras.get("checkup_strikes") or []):
            attrs = "".join(
                f"\t\t\t\t{k}={_wml_bool(v) if isinstance(v, bool) else v}\n"
...
def _strip_comment_lines(text: str) -> str:
    ...
    Keep `#textdomain` directives -- those are real WML. Wesnoth's
    parser accepts `#` comments outside macro-definition blocks, but
    we strip them for cleanliness since they can leak between blocks
    when we splice templates together."""
    out_lines = []
...
def _read_template(path: Path) -> str:
    ...
    lines. See `_strip_comment_lines`."""
...
def _render_side_block(
    ...
    for vx, vy in pre_placed_villages:
        ...
        parts.append(f"\t\ty={vy}")
        parts.append("\t[/village]")
    # Pre-placed army units (capability drills). random_traits=no
    # mirrors the drill .cfg: the sim models placed units trait-less,
    # so playback must instantiate them trait-less too.
    for utype, ux, uy in pre_placed_units:
        parts.append("\t[unit]")
        parts.append(f'\t\ttype="{utype}"')
        parts.append(f"\t\tside={side_num}")
        parts.append(f"\t\tx={ux}")
        parts.append(f"\t\ty={uy}")
        parts.append("\t\trandom_traits=no")
        parts.append("\t[/unit]")
    # No explicit [unit] block FOR THE LEADER: Wesnoth instantiates it from
...
def _scrape_scenario_preplaced_units(
    cfg_path: Path,
) -> Dict[int, List[Tuple[str, int, int]]]:
    """Per-side pre-placed `[unit] type=... x=N y=N` entries from a
    scenario .cfg's [side] 1/2 blocks (the capability drills place
...
def _per_scenario_template_path(scenario_id: str) -> Path:
    """`tools/templates/scenarios/<scenario_id>.wml` -- the cached
    save-shaped [scenario] block (post-macro expansion). Generated
...
def build_save_wml(
    ...
    *,
    ...
    Inputs: `sim.scenario_id` selects the per-scenario template
    under `tools/templates/scenarios/`. `sim.gs` selects per-side
    leader / faction / recruit / starting gold. `sim.command_history`
    becomes the [replay] block.

    The per-scenario template carries the FULL macro-expanded
    [scenario] block from a real Wesnoth save: map_data, ToD
    schedule, music playlist, prestart events (PLACE_IMAGE halos
    for petrified Basilisks, FLASH_WHITE+QUAKE for Aethermaw
    morphs, illuminate halos for Sulla's Ruins, etc.), per-side
    turn events (Aethermaw terrain mutations on turns 4-6), all
    expanded. The runtime emitter loads it, inserts our per-side
    [side] blocks (rendered from sim state) just before
    [/scenario], and stitches the whole thing into the outer
    save WML scaffold.

    The per-scenario templates are repo-tracked outputs of
    `tools/build_scenario_templates.py` (the game's own scenario
    .cfg expanded by the game's own preprocessor), so the chain is:
    wesnoth_src cfg + wesnoth.exe -p -> templates -> runtime.
    Covers the ladder pool AND the Mini Maps Collection pool.
    """
    scenario_id = sim.scenario_id
    template_path = _per_scenario_template_path(scenario_id)
    if not template_path.exists():
        raise RuntimeError(
            f"no per-scenario template at {template_path}. "
            f"Run `python tools/build_scenario_templates.py` "
            f"to regenerate from the scenario .cfg files."
        )
    ...
    # to PvP default. Scrape from .cfg rather than the template
    # because the template's stripped [side] blocks could have
    # been a non-default starting gold for that source replay's
    # game.
    cfg_path = _scenario_cfg_path(scenario_id)
    cfg_gold: Dict[int, int] = (
        _scrape_scenario_starting_gold(cfg_path) if cfg_path else {})
    # Pre-placed villages and army units also from the .cfg.
    village_map = (_scrape_scenario_villages(cfg_path)
                   if cfg_path else {1: [], 2: []})
    unit_map = (_scrape_scenario_preplaced_units(cfg_path)
                if cfg_path else {1: [], 2: []})
    # Keep positions from the template's map_data (faster than
    # hitting the .map again).
    template_text = template_path.read_text(encoding="utf-8")
    template_body = _strip_comment_lines(template_text)
    md_m = re.search(
        r'map_data\s*=\s*"((?:[^"\\]|\\.)*)"',
        template_body,
    )
    keep_pos: Dict[int, tuple] = {}
    if md_m:
    ...
    n_m = re.search(r'^\s*name\s*=\s*_?\s*"([^"]*)"',
