...
- **The claim that TCS never grades against an unsalted rollout** (which the luck ledger's unbiasedness silently depends on) held under inspection — every accept/reject is made on salted materializations (`turn_search.py:520-521`, `:551-578`) — but it is undocumented. If the ledger ships, write it into `docs/archive/tcs_spec.md` as an invariant.
