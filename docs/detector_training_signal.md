...
## Campaign pre-flight (2026-07-25) — READ BEFORE LAUNCHING

Training-loop telemetry now logs the signal's health every train_step
(`TrainStats.advice_*` -> the `iter N: train_step` line): `advice_fire`
(fraction of states carrying advice), `advice_opps`, `advice_grad_share`
(||g_advice||/||g_total||, measured BEFORE clipping) and `advice_out_norm`
(the zero-init bootstrap tracker).

Wiring it up immediately surfaced a launch-blocking caveat:

- **Local self-play measured `advice_fire = 0.000`** (0/348 with MCTS-4,
  then 0/863 raw-policy over 2 full 30-turn games) while real ladder states
  fire at 14%. Root cause: **those games had NO ARMY** -- a 31-turn game
  ended with `units=2`, i.e. just the two leaders. The raw / low-budget
  policy recruits nothing, so there is never an adjacency for any motif to
  fire on. This is the gold-hoarding / under-recruiting regression
  (BACKLOG 2026-07-20) in its most extreme form, NOT an advice bug: the
  advisor, the trainer path and `current_side` were all verified correct
  (forcing the side gives the same 14% on ladder states).
- **Implication for the campaign:** the advice signal's fire rate is
  downstream of the policy actually fielding and engaging an army. The 14%
  estimate comes from HF bundles generated by a REAL campaign (proper
  search budget), so a full-budget campaign should exercise the signal --
  but this cannot be confirmed on the dev box (32-sim self-play is far too
  slow locally).
- **Therefore: watch `advice_fire` in the first iterations.** If it reads
  ~0, the run is not exercising the signal at all (no army / no contact)
  and should be stopped rather than burning GPU hours -- the fix would be
  the recruiting regression, not the advice path. A healthy run should
  approach the ~13-14% offline baseline, with `advice_out_norm` growing off
  zero (the bootstrap) and `advice_grad_share` in the percent range.
