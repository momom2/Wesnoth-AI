...
(the match's fit, the holdout curve, the per-phase value table, the box
facts and progress); the logs (training, match, pre-encoding, tests,
escrow, stop) and the per-phase JSON are on HF under
`tier-b/observation_retrain_20260924/`, with the arm's checkpoint
(`arm_epoch0.pt`) and the 800 games' result files and whole-game
records (`games_obs_e1_vs_terrain.tar.gz`).
...
measured yet: the two maps whose turn number misleads about the time of
