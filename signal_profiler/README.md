...
- the imitation trainer (`tools/supervised_train.py`): a row every
  25,000 trained pairs in `<checkpoint stem>_signal.jsonl`. A probe of
  32 pairs of the batch just trained is split by loss term (actor,
  ...
  tests/test_signal_telemetry.py holds the weights bit-identical on
  CPU. `read_signal_rows` reads a file whose run was resumed.
